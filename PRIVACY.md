...
|---|---|---|
| **The placeholder mapping** — every placeholder and the name it stands for, filed under the project it belongs to, plus the words you have typed into the preview's rename column | `snippetveil-placeholders.xml` in the IDE's configuration directory | Outside the project tree, so it cannot be committed; outside the cache tier, so *Invalidate Caches* cannot destroy your ability to read an old reply |
