...
/**
 * **What the platform says about one declared symbol, in the shapes the plan reports it in.**
 *
 * Every function here answers a question about a `PsiElement` — what its key is, what it is in the
 * grammar, where its declaring file lives — and none of them is about a *language*. That is why they
 * are here rather than in either walk: a Kotlin declaration reaches the ledger as the light element a
 * Java reference to it would resolve to, so **the two walks have to agree on a key character for
 * character** or one symbol gets two placeholders and the shared ledger comes apart.
 *
 * A second copy of these formats living beside the Kotlin walk would be exactly that failure waiting
 * for the day one copy is edited. So there is one copy, and both walks call it.
 *
...
internal object SymbolFacts {
    ...
    fun overrideRootsOf(method: PsiMethod, ownerOf: (PsiElement) -> SymbolOrigin): List<OverrideRoot> =
        method.findDeepestSuperMethods().map {
            OverrideRoot(keyOf(it), ownerOf(it), packageNameOf(it), keyIsQualified(it))
        }
